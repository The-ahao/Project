...
namespace ZZZ
{
    public class CharacterComboBase
    {
        ...
        {
            SFXPoolManager.MainInstance.TryGetSoundPool(reusableData.currentCombo.GetComboSoundStyle(reusableData.currentIndex.Value), playerTransform.position, Quaternion.identity);
        }
        ...
        private void AttackTrigger()
        {
            ...
            else if (animator.AnimationAtTag("Skill"))
            {
                ...
                if (reusableData.canQTE && reusableData.ATKIndex >= reusableData.currentSkill.attackCount)
                {
                    TriggerSwitchSkill();
                }
                #endregion
        ...
        {
            SFXPoolManager.MainInstance.TryGetSoundPool(SoundStyle.ComboVoice,comboData.comboName, playerTransform.position);
        }
        protected void PlayWeaponSound(ComboData comboData)
        {
            SFXPoolManager.MainInstance.TryGetSoundPool(SoundStyle.WeaponSound, comboData.comboName, playerTransform.position);
        }
