...
namespace ZZZ
{
    public class CharacterCombo : CharacterComboBase
    {
        ...
        {
            //放慢时间，但是还不能输入
            SFXPoolManager.MainInstance.TryGetSoundPool(SoundStyle.SwitchTime,player.characterName.ToString(),playerTransform.position); 
            CameraHitFeel.MainInstance.StartSlowTime(0.06f);
            TimerManager.MainInstance.GetScaleTimer(0.2f, StartSwitchSkill);
        }
        /// <summary>
        /// 启动切换技能功能
        /// 此方法用于初始化技能切换的UI和输入监听
        /// </summary>
        protected void StartSwitchSkill()
        {
            //QTE倒计时
            TimerManager.MainInstance.GetScaleTimer(3, CancelSwitchSkill);
            //激活UI
